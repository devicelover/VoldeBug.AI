...
const T = {
  en: {
    ...
    myClasses: 'My classes', createClass: 'Create a class', className: 'Class name',
    classNameErr: 'Give the class a name', classCreated: 'Class created',
    classCreateForbidden: 'Sign in with a teacher account to create a class',
    noClassesYet: 'No classes yet',
    noClassesSub: 'Create one and share its code — students who type it in appear here.',
    rosterLoading: 'Loading your class…',
    rosterUnavailable: 'Could not load this class right now.',
    rosterEmpty: 'Nobody has joined yet',
    rosterEmptySub: 'Share this code with your students:',
  ...
  hi: {
    ...
    myClasses: 'मेरी कक्षाएँ', createClass: 'कक्षा बनाएँ', className: 'कक्षा का नाम',
    classNameErr: 'कक्षा को नाम दें', classCreated: 'कक्षा बन गई',
    classCreateForbidden: 'कक्षा बनाने के लिए शिक्षक खाते से साइन इन करें',
    noClassesYet: 'अभी कोई कक्षा नहीं',
    noClassesSub: 'एक कक्षा बनाएँ और उसका कोड साझा करें — जो विद्यार्थी कोड डालेंगे वे यहाँ दिखेंगे।',
    rosterLoading: 'आपकी कक्षा लोड हो रही है…',
    rosterUnavailable: 'अभी यह कक्षा लोड नहीं हो सकी।',
    rosterEmpty: 'अभी कोई नहीं जुड़ा',
    rosterEmptySub: 'यह कोड अपने विद्यार्थियों को दें:',
...
function forceSignOut(announce) {
  ...
  // Classmates' names belong to the signed-in session only. Same for a
  // teacher's roster and a principal's school numbers: both are other
  // people's data, and neither may survive the account they came from.
  try { liveBoard = null; } catch {}
  try { teacherClasses = null; selectedClassCode = ''; classRosters = {}; } catch {}
  try { schoolStats = null; } catch {}
...
/* `skipClassCode` exists for the onboarding path: /journey/onboard stores
   whatever code it is handed without checking it against a real class, so a
   student who typed a guess would end up parked in a class that does not
   exist. Onboarding therefore syncs everything else first and puts the code
   through the validated class_join event instead. */
async function syncOnboard(opts) {
  if (!signedIn()) return;
  try {
    const body = {
...
/* Joining a class, for a signed-in student, is the one place the app waits on
   the server before believing itself: the code is a shared secret a teacher
   read out, and a typo used to leave the student sitting in a class that was
   never real, wondering why the board stayed empty. The server answers with
   accepted:0 for a code it does not recognise.

   `accepted` is also 0 when the student is already in that class (the server
...
/* Live board: real classmates from the server once a signed-in student has
   joined a class with at least one other member. Anything short of that keeps
   the illustrative PEERS list, so the screen never renders empty or lonely.
   Cached for a minute — rank changes don't need to be more real-time than
   the next visit to this tab. */
let liveBoard = null;
let liveBoardLoading = false;

function avatarEmoji(key) { const a = AVATARS.find(x => x.k === key); return a ? a.e : '🦊'; }
...
function fetchLiveBoard() {
  ...
  api('/journey/leaderboard').then(p => {
    ...
    if (d && d.scope === 'class' && Array.isArray(d.entries) && d.entries.length > 1) {
      ...
      if (currentRoute() === 'board') render();
    }
  }).catch(() => {}).finally(() => { liveBoardLoading = false; });
}

function viewBoard() {
  const live = liveBoard && signedIn() ? liveBoard.entries : null;
  const weekly = !live && S.boardWeek;
  const av = AVATARS.find(a => a.k === S.avatar) || AVATARS[0];
  const me = { n: (S.name.trim() || 'You'), a: av.e, xp: S.xp, w: Math.round(S.xp * 0.22), me: true };
  const all = live
    ? live.map(e => ({ n: e.isMe ? (S.name.trim() || 'You') : e.name, a: avatarEmoji(e.avatar), xp: e.xp, w: 0, me: e.isMe }))
    : [...PEERS, me].sort((a, b) => (weekly ? b.w - a.w : b.xp - a.xp));
  ...
  return `<div class="view">
    <section class="card reveal" style="text-align:center">
      ...
      <div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;margin-top:12px">
        ...
        ${S.classCode ? `<span class="chip chip--sky">🏫 ${esc(S.classCode.toUpperCase())}</span>` : ''}
      </div>
    </section>
...
function afterProfile() {
  ...
  if (jb) jb.onclick = async () => {
    ...
    // Signed in: the server decides. A mistyped code used to be accepted
    // locally and quietly ignored by the backend, leaving a student in a
    // class chip that pointed at nothing.
    if (signedIn()) {
      jb.disabled = true;
    ...
    // Guest: unchanged — local only, no network, no gate.
    S.classCode = v; save();
    pushEvent('class_join', { code: v });
...
/* Real classes for a signed-in teacher. Everything here degrades to the
   ROSTER sample above: a teacher who has not signed in, or whose network is
   down, sees exactly the screen they saw before this existed, because a
   staffroom demo on school wifi must not be able to fail. Cached for a
   minute like the student leaderboard — a roster is not a live feed.
...
function sampleChip() {
  return `<span class="chip chip--volt" style="font-size:11px">${S.locale === 'hi' ? 'नमूना' : 'sample'}</span>`;
}
...
function fetchTeacherClasses(force) {
  ...
  api('/journey/class').then(p => {
    ...
    // Keep the teacher's choice across refreshes; fall back to the newest
    // class when the selected one is gone (or nothing was selected yet).
    if (!selectedClassCode || !list.some(c => c.code === selectedClassCode)) {
...
function teacherSelectedClass() {
  const list = teacherLiveClasses();
  if (!list || !list.length) return null;
  return list.find(c => c.code === selectedClassCode) || list[0];
}

/* Card listing the teacher's own classes. Only ever rendered signed in. */
function teacherClassesCard() {
  const list = teacherLiveClasses();
  const sel = teacherSelectedClass();
  const head = `<div class="sec-head" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
  ...
  if (!list) {
    return `<section class="sec reveal">${head}
      <div class="card"><p class="muted" style="font-weight:700;font-size:14px">${esc(t('rosterLoading'))}</p></div>
    </section>`;
  }
    ...
    return `<section class="sec reveal">${head}
      <div class="card">
        <div class="empty-state">
          <div class="empty-state__emoji">🏫</div>
          <p class="empty-state__title">${esc(t('noClassesYet'))}</p>
          <p class="empty-state__sub">${esc(t('noClassesSub'))}</p>
        </div>
        <button class="btn btn--primary btn--block" id="newClass">＋ ${esc(t('createClass'))}</button>
      </div>
    </section>`;
  }

  return `<section class="sec reveal">${head}
    <div class="card" style="padding:8px;display:flex;flex-direction:column;gap:8px">
      ${list.map(c => {
        const on = sel && c.code === sel.code;
        const up = String(c.code).toUpperCase();
        return `<div style="display:flex;align-items:center;gap:8px;padding:8px 10px;border:1px solid ${on ? 'var(--violet)' : 'var(--border)'};border-radius:var(--r);background:var(--surface-2)">
...
/* Name + age band, nothing else: a class code is generated by the server, so
   there is nothing for the teacher to invent or mistype here. */
function openCreateClassSheet() {
  let grp = S.classGroup || 'middle';
  sheet(`
    <h2 style="font-size:21px;margin-bottom:6px">🏫 ${esc(t('createClass'))}</h2>
    <div class="field">
      <label for="clsName">${esc(t('className'))}</label>
      <input class="input" id="clsName" maxlength="60" autocomplete="off"/>
      <p class="field__err" id="clsErr"></p>
    </div>
    <p class="muted" style="font-weight:800;font-size:12.5px;letter-spacing:.6px;text-transform:uppercase;margin-bottom:8px">${esc(t('classGroupLabel'))}</p>
    <div class="segment" style="margin-bottom:16px">
      ${Object.keys(CLASS_GROUPS).map(k => `<button class="${k === grp ? 'on' : ''}" data-grp-pick="${k}">${CLASS_GROUPS[k].emoji} ${esc(L(CLASS_GROUPS[k].label))}</button>`).join('')}
    </div>
    <button class="btn btn--primary btn--block" id="clsGo">${esc(t('createClass'))}</button>
  `, (node, close) => {
    const nameEl = node.querySelector('#clsName');
    const errEl = node.querySelector('#clsErr');
    const goBtn = node.querySelector('#clsGo');
    nameEl.focus();

    node.querySelectorAll('[data-grp-pick]').forEach(b => b.onclick = () => {
      grp = b.dataset.grpPick;
      node.querySelectorAll('[data-grp-pick]').forEach(x => x.classList.toggle('on', x === b));
      haptic();
    });
    ...
    goBtn.onclick = async () => {
      ...
      if (!name) { errEl.textContent = t('classNameErr'); haptic(60); return; }
      busy = true; errEl.textContent = '';
      goBtn.disabled = true; goBtn.textContent = t('cloudWorking');
      try {
        const out = await api('/journey/class', { method: 'POST', body: { name, classGroup: grp } });
        const cls = out && out.data && out.data.class;
        close();
        if (cls && cls.code) {
          selectedClassCode = cls.code;
          toast('🏫 ' + t('classCreated') + ' · ' + String(cls.code).toUpperCase());
          haptic(18); chime([659.25]);
        }
...
function teacherRosterRows(members) {
...
function viewTeacher() {
  const sel = teacherSelectedClass();
  const roster = sel ? classRosters[sel.code] : null;
  const live = roster && !roster.failed ? roster.members : null;
  const myUpskill = UPSKILL_MODULES.filter(m => S.teacherModulesDone.includes(m.key)).length;

  // Headline numbers follow whatever the table below is showing, so the two
  // can never disagree — including in the gap between picking a real class
  ...
  return `<div class="view">
    ...
    ${signedIn() ? teacherClassesCard() : ''}

    ${noClasses ? '' : `<section class="card codebox reveal">
      <p class="codebox__label">${esc(sel ? sel.name : t('classCodeLabel'))}</p>
    ...
    <section class="sec stats stagger">
      <div class="card stat" style="--i:0"><div class="stat__k">${count}</div><div class="stat__l">${esc(t('students'))}</div></div>
    ...
    <section class="sec reveal">
      <div class="sec-head" style="display:flex;align-items:center;gap:10px">
        <h2 style="flex:1">${esc(t('students'))}</h2>
        ${(!live && !noClasses && !pending) ? sampleChip() : ''}
      </div>
      <div class="card" style="padding:6px">
        ${teacherRosterBody(sel, roster, live)}
      </div>
    </section>

    ${signedIn() ? '' : `<p class="muted" style="font-weight:700;font-size:13.5px;line-height:1.5;margin-top:14px">☁️ ${esc(t('teacherCloudHint'))}</p>`}
  </div>`;
}

/* Split out because the roster has four honest states — no class yet, still
...
function teacherRosterBody(sel, roster, live) {
  ...
  if (live && !live.length) {
    return `<div class="empty-state">
      <div class="empty-state__emoji">📣</div>
      <p class="empty-state__title">${esc(t('rosterEmpty'))}</p>
      <p class="empty-state__sub">${esc(t('rosterEmptySub'))}</p>
      <p class="mono" style="font-size:26px;font-weight:700;letter-spacing:6px;margin-top:10px">${esc(String(sel.code).toUpperCase())}</p>
    </div>`;
  }

  const rows = live
    ? teacherRosterRows(live)
    : ROSTER.map(s => {
  ...
  return `<div class="tbl__scroll"><table class="tbl">
    <thead><tr>
...
function afterTeacher() {
  ...
  document.querySelectorAll('[data-cls]').forEach(b => b.onclick = () => {
    if (selectedClassCode === b.dataset.cls) return;
    selectedClassCode = b.dataset.cls;
    haptic();
    render();
  });
...
/* School-wide numbers for a signed-in principal. Same rules as the teacher
   roster: cached for a minute, never awaited by a render, and a 403 (an
   account that is not registered as a principal) or a dead network leaves
   the illustrative SCHOOL_DATA screen exactly as it was. */
let schoolStats = null;          // { data, at }
let schoolStatsLoading = false;

function fetchSchoolStats() {
  ...
  api('/journey/school').then(p => {
    ...
    if (d && typeof d.students === 'number') {
      ...
      if (currentRoute() === 'principal') render();
    }
  }).catch(() => {}).finally(() => { schoolStatsLoading = false; });
}

function viewPrincipal() {
  const sv = signedIn() && schoolStats ? schoolStats.data : null;
  const liveClasses = sv && Array.isArray(sv.classes) ? sv.classes : null;
  ...
  // Server classes have no teacher name and no per-class XP average, so the
  // live banner leans on level and weekly activity instead of inventing one.
  const liveTop = liveClasses && liveClasses.length
    ? [...liveClasses].sort((a,b)=>b.avgLevel-a.avgLevel)[0] : null;

  return `<div class="view">
    ${sv
      ? `<p class="chip chip--flame" style="margin-bottom:14px">● ${esc(t('principalLive'))}</p>`
      : `<div class="demo-note">🧪 ${esc(t('demoNote'))}</div>`}
    ...
    ${sv && !liveTop ? '' : `<div class="card reveal" style="background:linear-gradient(135deg,var(--ink),var(--violet));color:#fff;margin-bottom:20px;padding:20px">
    ...
    <div class="sec-head">
      <h2 style="flex:1">${esc(t('staffProgress'))}</h2>${sv ? sampleChip() : ''}
      <p>${esc(t('staffProgressSub'))}</p>
    </div>
...
function afterPrincipal() {
  /* Nothing to wire — the report/coverage links use the shared [data-go]
     ...
     that opens the real thing. The one job left is asking the server for
     real numbers; the mock is already on screen by the time it answers. */
  fetchSchoolStats();
}
...
function afterOnboarding() {
  ...
  const finish = () => {
    ...
    // A class code typed here goes through the validated join path rather
    // than straight into the onboard payload, which does not check it.
    if (signedIn() && S.classCode) verifyOnboardClassCode();
    else syncOnboard();
